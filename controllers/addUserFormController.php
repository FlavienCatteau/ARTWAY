...
if ($_POST['registerForm'] ?? '') {
    ...
    // Si le champs pseudo est vide
    if (!is_numeric($_POST['service']) || $_POST['service']<1 || $_POST['service']>2) {
    ...
    if (empty($formError)) {
        try {
            $User = new User();
            //on appelle la methode qui insere l'utilisateur dans la BDD puis on detruit l'objet user_BDD
